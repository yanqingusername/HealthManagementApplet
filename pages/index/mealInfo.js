...
Page({
    ...
    var data = {
     ...
     if((data.meal_content_final==""&&data.intake_amount_final=="")&&data.imgArr.length==0){
       ...
       return
          ...
          // return
      //  box.showToast("ok");
         request.request_get('/api/updatFoodInfo.hn', data, function (res) {
           console.info('回调', res)
           if (res) {
             if (res.success) {  
               var id = res.msg
  ...
  getMealInfo: function () {
    var that = this;
    var record_num = that.data.record_num;
    request.request_get('/api/getMealById.hn', {
      record_num: record_num
    }, function (res) {
      console.info('getMealInfo回调', res)
      console.info("修改",res);
      if (res) {
        if (res.success) {
          var info = res.msg;
          var imgArr = info.img_url;
          var meal_content_begin = info.meal_content;
          var intake_amount_begin = info.intake_amount;
          var intake_company_begin = info.intake_company_final;
          var meal_content_arr = meal_content_begin.split(";");
          meal_content_arr.pop();
          var intake_amount_arr = intake_amount_begin.split(";");
          intake_amount_arr.pop();
          var intake_company_arr = intake_company_begin.split(";");
          intake_company_arr.pop();
          var info_p = new Info("","","");
          console.log(info_p)
          for(var i = 0;i <meal_content_arr.length;i++){
            info_p.details.push(new Detail(meal_content_arr[i],intake_amount_arr[i],intake_company_arr[i]))
          }
          console.log(info_p)
          if(info.meal_type== 1){
            that.setData({ 
        ...
        }else if(info.meal_type== 4){
            that.setData({ 
                meal_type: '加',
                meal_type_id:4
            })
      }
         var time=info.mealTime;
         time=time.split(" ");
         console.info("接收的时间",time);
          that.setData({
            startTime:info.mealTime,
            img_arr: imgArr, //食物图片
            info:info_p,
            yearmouthday:time[0],
            hoursminute:time[1]
